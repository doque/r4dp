...
class RequestValidator {
	...
	public function notEmpty($field, $value, $message = '') {
		if(trim($value) === '') $this->triggerError($field, $message);
		return !$this->hasError($field);
	}

	public function inArray($field, $needle, $haystack, $message = '') {
		if(!in_array($needle, $haystack)) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function inArrayKeys($field, $needle, $haystack, $message = '') {
		if(!array_key_exists($needle, $haystack)) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function minLength($field, $value, $min, $message = '') {
		if(strlen($value) < $min) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function maxLength($field, $value, $max, $message = '') {
		if(strlen($value) > $max) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function rangeLength($field, $value, $min, $max, $message = '') {
		$len = strlen($value);
		if($len < $min || $len > $max) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function length($field, $value, $length, $message = '') {
		if(strlen($value) != $length) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function min($field, $value, $min, $message = '') {
		if(intval($value) < $min) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function max($field, $value, $max, $message = '') {
		if(intval($value) > $max) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function range($field, $value, $min, $max, $message = '') {
		$value = intval($value);
		if($value < $min || $value > $max) $this->triggerError($field, $message);
		return !$this->hasError($field);
	}
 
	public function equal($field, $value, $compare, $message = '', $strict = false) {
		if($strict) {
			if($value !== $compare) $this->triggerError($field, $message);
		} else {
			if($value != $compare) $this->triggerError($field, $message);
		}
		return !$this->hasError($field);
	...
	public function hasError($field) {
		if(!isset($this->errors[$field])) return false;
		return true;
	}
}
